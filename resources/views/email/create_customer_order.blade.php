            	...
            	<table class="main-wrap" width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff">
                    ...
                    <tr>
                    	<td align="center" style="font-size: 24px; font-weight: 200; font-family: Open sans, Helvetica, Arial, sans-serif; color: #222222;">
                        	Congratulations <em>{{!empty($details['customer_details']->shipping_first_name) ? $details['customer_details']->shipping_first_name : ''}}</em>!
                        </td>
                 	</tr>
                    <tr>
					...
					<tr>
                    	<td align="center" style="background: #7dc85533; font-size: 18px; font-weight: 200; font-family: Open sans, Helvetica, Arial, sans-serif; color: #222222; padding-top: 10px; padding-bottom: 10px;">
							...
							<p style="font-size: 13px; padding: 10px;">Your order number is <a href="{{url('/order-complete/'.$details['session_order_data']->order_id)}}">#pet-{{$details['session_order_data']->order_id}}</a>.
                    	...
                    	<td align="right" style="font-size: 14px; font-weight: 400; font-family: Open sans, Helvetica, Arial, sans-serif; color: #222222; padding-top: 10px; padding-left: 20px; padding-right: 20px;">
							<div class="ship-to" style="text-align:left">
								...
								<span style="font-size:13px; line-height:20px;">
									<strong style="font-size:16px; color: #777; font-weight: 200;">{{!empty($details['customer_details']->shipping_first_name)? $details['customer_details']->shipping_first_name : ''}} {{!empty($details['customer_details']->shipping_last_name)? $details['customer_details']->shipping_last_name : ''}}</strong><br>
									<b>Address:</b> {{!empty($details['customer_details']->address)? $details['customer_details']->address : ''}}  <br>
									<b>Phone:</b> {{!empty($details['customer_details']->contact_number)? $details['customer_details']->contact_number : ''}}  <br>
									<b>City:</b> {{!empty($details['customer_details']->shipping_city)? $details['customer_details']->shipping_city : ''}}<br>
									<b>Email:</b> {{!empty($details['customer_details']->shipping_email)? $details['customer_details']->shipping_email : ''}}
								</span>
							</div>
							<div class="order-details" style="text-align:left">
								...
								<span style="font-size:13px; line-height:18px;">
									<b>Order Date:</b> {{$details['session_order_data']->order_data->created_at}}<br>
									<b>Order ID:</b> #<a href="{{url('/order-complete/'.$details['session_order_data']->order_id)}}">pet-{{$details['session_order_data']->order_id}}</a><br>
									<b>Order Total Items:</b> {{$details['session_order_data']->order_data->total_items}}<br>
									<b>Order Total Qty:</b> {{$details['session_order_data']->order_data->total_qty}}<br>
									<b>Order Status:</b> {{$details['session_order_data']->order_data->order_status}}<br>
									<b>Status Message:</b> {{$details['session_order_data']->order_data->order_message}}<br>
								</span>
							</div>
							...
							<table class="product-detail" width="100%" >
								...
								<tbody>
									<tr>
										<td colspan="3" width="100%">
											<table width="100%">
												...
												@foreach ($details['session_order_data']->order_items as $item)
												<tr>
													<td align="right"><img src="{{$item->image}}" alt="Petssified" class="logo" style="display: block; width: 96%" /></td>
													<td align="right">
														<a href="#" target="_blank" title="{{$item->product_title}}">{{$item->product_title}}</a><br>
														(${{$item->price}} * <strong>{{$item->p_qty}}</strong>)
													</td>
													<td align="right">-${{$details['session_order_data']->order_data->discount_amount}}</td>
													<td align="right"> {{($item->price * $item->p_qty)}}$ </td>
												</tr>
												@endforeach
											</table>
										</td>
									...
									<tr>
										...
										<td align="right">US: ${{$details['session_order_data']->order_data->total_price}}$</td>
									</tr>
									<tr>
										<th colspan="2" align="right">Shipping</th>
										<td align="right">US: {{$details['session_order_data']->order_data->shipping_price}}$</td>
									</tr>
									<tr>
										<th colspan="2" align="right">Tax</th>
										<td align="right">US: {{$details['session_order_data']->order_data->tax}}$</td>
									</tr>
									<tr>
										<th colspan="2" align="right" style="border-top: 2px solid #aaa;border-bottom: 1px solid #ddd;">Total</th>
										<th align="right" style="border-top: 2px solid #aaa;border-bottom: 1px solid #ddd;">US: {{$details['session_order_data']->order_data->final_amount}}$</th>
									</tr>
								</tbody>
							</table>
