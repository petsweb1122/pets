            ...
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    @if (!empty($categories_mapped))
                        {!! Form::open(['url' => 'admin/upload-products/phillipspet-by-api']) !!}
                        {!! Form::submit('Sync', ['class' => 'btn btn-primary']) !!}
                        {!! Form::close() !!}
                    @else
                    <label class="label label-danger" for="">Need To Map the Categories, go Categories Validation Module and map the categories</label>
                    @endif

                </div>
                <div class="form-group col-md-6">
                    {!! Form::open(['url' => 'admin/upload-products/sync-phillipspet-categories-by-api']) !!}
                    <label for="">Click Sync Button And sync all the Categories</label>
                    <br>
                    {!! Form::submit('Sync Categories', ['class' => 'btn btn-primary']) !!}
                    {!! Form::close() !!}
                </div>
                <div class="form-group col-md-6">
                    {!! Form::open(['url' => 'admin/upload-products/fetch-phillipspet-by-api']) !!}
