...
<section class="invoice">
    ...
    <div class="row">
        <div class="col-xs-12">
            <h2 class="page-header">
                ...
                <small
                    class="pull-right">{{ Carbon\Carbon::parse(!empty($order_obj->created_at) ? $order_obj->created_at : '')->format('d M Y') }}</small>
            </h2>
        </div>
        <!-- /.col -->
    </div>
    ...
    <div class="row invoice-info">
        <div class="col-sm-4 invoice-col">
            Ship To:
            <address>
                <strong>Petssified{{ !empty($order_details[0]->customer_first_name) || !empty($order_details[0]->customer_last_name) ? $order_details[0]->customer_first_name . ' ' . $order_details[0]->customer_last_name : '' }}</strong><br>

                <b>Address:</b>
                3773 Howard Hughes Pkwy, Las Vegas, NV 89169, United States{{ !empty($order_details[0]->customer_address) ? $order_details[0]->customer_address : '' }}<br>
                <b>Phone:</b>
                +1-213-442-5200{{ !empty($order_details[0]->customer_contact_number) ? $order_details[0]->customer_contact_number : '' }}<br>
                <b>City:</b>
                Las Vegas{{ !empty($order_details[0]->customer_city) ? $order_details[0]->customer_city : '' }}<br>
                <b>Email:</b>
                vendor@petssified.com{{ !empty($order_details[0]->customer_email) ? $order_details[0]->customer_email : '' }}
            </address>
        </div>
        <!-- /.col -->
        <div class="col-sm-4 invoice-col pull-right">
            ...
            {{ !empty($order_obj->total_items) ? $order_obj->total_items : '' }}<br>
            <b>Order Total Quantites</b>
            {{ !empty($order_obj->total_qty) ? $order_obj->total_qty : '' }}<br>
            <b>Order Status:</b>
            {{ !empty($order_obj->order_status) ? ucwords(str_replace('-', ' ', $order_obj->order_status)) : '' }}<br>
            <b>Status Message:</b>
            {{ !empty($order_obj->order_message) ? ucwords($order_obj->order_message) : '' }}<br>
        </div>
        <!-- /.col -->
    </div>
    <!-- /.row -->
                ...
                <tbody>
                    @foreach ($order_details as $item)
                        <tr>
                            ...
                            @endphp
                            <td>{{ $item->upc }}</td>
                            <td><a href="{{ url("/$title_link") }}" target="_blank"
                                    title="{{ $item->product_title }}">{{ $item->product_title }}</a></td>
                            <td>{{ $item->vendor_name }}</td>
                            <td>{{ $item->order_product_price }}</td>
                            <td>{{ $item->order_product_qty }}</td>
                            <td>US: {{ $item->order_product_price * $item->order_product_qty }}$</td>
                        </tr>
                    @endforeach
            ...
            <div class="table-responsive">
                <table class="table">
                    <tr>
                        ...
                        <td>US: {{ number_format($order_obj->total_price, 2) }}$</td>
                    </tr>
                    <tr>
                        <th style="width:50%">Shipping Price:</th>
                        <td>US: {{ $order_obj->shipping_price }}$</td>
                    </tr>
                    <tr>
                        <th>Total:</th>
    ...
    @if ($user_data->role == 'super-admin' || $user_data->role == 'vendor')
        <div class="row">
            <div class="col-md-12">
                {!! Form::open(['url' => 'admin/vendor-orders/'.request()->vendor_id.'/update-order/'.request()->order_id]) !!}
                @php
